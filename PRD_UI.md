...
게임에서 주황/골드는 "선택됨"(`ノーマル` 탭)이고 앱에서는 "경고"(`⚠ 글자 상태`)다. **분리하기로 했다** — 선택은 `Accent.Orange`, 경고는 별도 `Gold.*` 계열로 두어 앱의 의미를 보존한다. 크림 전환 시 `Gold.Text`는 밝은 크림골드(`#E0C978`)에서 **어두운 딥앰버(`#8A5A00` 계열)로 반전**한다. 경고 배너만 다크로 남기는 안도 검토했으나, 다른 배너(청록·자주)와 문법이 어긋나서 채택하지 않았다.
